...
【输出格式（务必严格遵守，分两段输出）】
第一段：机器可读 JSON。必须用 \`\`\`json 代码块包裹，放置在最前面，且只能出现一次。
代码块内严格只输出 JSON，不要写注释、不要省略字段、不要多余文字。
JSON Schema 如下（verdict/severity 必须是下面列出的英文枚举值）：
{
  "verdict": "pass" | "warn" | "block",
  "verdict_label": "可上线 / 有非阻塞问题，但建议修 / 有阻塞问题，暂不可上线",
  "summary": "一句话总结本次测试的整体情况",
  "issues": [
    {
      "severity": "high" | "medium" | "low",
      "title": "一句话描述问题",
      "detail": "2-3 句话说明影响范围与触发条件",
      "file": "问题所在文件:行号（可选，无法定位就填空字符串）"
    }
  ]
}

verdict 判定标准：
- pass：未发现问题，或仅有低严重度问题，可直接上线
- warn：发现中等严重度问题，不阻塞上线但建议尽快修复
- block：发现高严重度问题（如核心功能不可用、安全漏洞、数据丢失等），暂不可上线

issues 数组：每条问题对应一个具体可修复的点，按严重度从高到低排序；没发现问题就输出空数组 []。

第二段：人类可读报告。紧跟 JSON 代码块之后输出，不要再输出任何 JSON。分节输出：
✅ 通过的检查
